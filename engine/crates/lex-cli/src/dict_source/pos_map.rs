...
/// - `7` (Counter): 助数詞 (e.g., 名詞,接尾,助数詞) — narrower than Suffix
...
mod tests {
    ...
    fn test_morpheme_roles() {
        ...
        assert_eq!(roles[2010], ROLE_COUNTER); // 名詞,接尾,助数詞 (generic)
        assert_eq!(roles[2013], ROLE_COUNTER); // 名詞,接尾,助数詞 (specific: 回)
